...
#
# Platform: Linux shell environment
# Run this script from a workstation or automation host with AWS CLI credentials configured.
#
# Usage: ./drift-check.sh <security-group-id>
# Example: ./drift-check.sh sg-xxxxxxxxxxxxxxxxx
#
# Prerequisites:
#   - aws cli
#   - jq
...
for arg in "$@"; do
    case $arg in
        ...
        sg-*)
            ...
            ;;
        --help|-h)
            echo "Usage: ./drift-check.sh [--debug] <security-group-id>"
            echo ""
            echo "Options:"
            echo "  --debug, -d    Show detailed diagnostic output at end"
            echo "  --help, -h     Show this help message"
            echo ""
            echo "Example: ./drift-check.sh --debug sg-xxxxxxxxxxxxxxxxx"
            exit 0
            ;;
    esac
done
...
if [ "$PREREQ_FAILED" = true ]; then
    ...
    echo ""
    echo "Usage: ./drift-check.sh <security-group-id>"
    echo ""
    exit 1
fi
...
if [ -z "$SECRET_ARN" ]; then
    ...
    echo "    arn = \"arn:aws:secretsmanager:<region>:<account-id>:secret:<secret-name>\""
    exit 1
fi
...
# Format: ip8 = jsondecode(...)["office-cidr-1"]
# Also parses commented lines like: # ip1 = jsondecode(...)["admin-cidr-1"]
echo ""
...
grep -E "ip[0-9]+\s*=" data.tf | while read -r line; do
    ...
    # Extract secret key (e.g., office-cidr-1)
...
*Generated by drift-check.sh - READ ONLY (no changes made to AWS)*
PRFOOTER
...
#
# SCRIPT INFO:
#   Generated by: drift-check.sh v${SCRIPT_VERSION}
#   Generated on: $(date)
#   Server: $(hostname)
#
# SECURITY GROUP:
